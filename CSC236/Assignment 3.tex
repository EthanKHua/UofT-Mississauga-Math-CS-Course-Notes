...
\begin{proof}
	...
	It has been proven that \(D'\) accepts \(\mathcal{L} (\overline{r})\), which shows that \(\overline{r}\) is a standard regex. Now, it will be shown using structural induction that all regex with access to complement match a regular language.

	\textbf{Base Case.} Let \(a \in \Sigma \). The regexes \(\varnothing, \varepsilon, a\) match regular languages.

	\textbf{Induction Hypothesis.} Suppose that \(r_1, r_2\) are regexes with access to complement such that they represent a regular language.

	\textbf{Induction Step.} By the induction hypothesis, \(\mathcal{L} (r_1)\) and \(\mathcal{L} (r_2)\) are regular. It follows that \(\mathcal{L} (r_1 + r_2)\), \(\mathcal{L} (r_1 r_2)\), and \(\mathcal{L} (r_1 ^*)\) are regular. As well, it is also true that \(\mathcal{L} (\overline{r_1})\) is regular.

	By the principle of structural induction, every regex with access to complement matches a regular language, and the proof is complete.
	\smallbreak
\end{proof}
...
\begin{proof}
	...
	\textbf{Base Case.} It is obvious that \(P_{q_\varepsilon}(\varepsilon)\) is true.

	\textbf{Induction Hypothesis.} For some string \(x\) of length less than or equal to \(k\), let \(q_x \in Q\). Let \(w \in \Sigma ^*\). Suppose that \(P_{q_x}(w)\) holds true, that is, if \(|x| < k\), then \(w=x\), or otherwise the \(k\) last characters of \(w\) are exactly \(x\).

	\textbf{Induction Step.} For all \(z \in \Sigma\), \(\delta (q_x, z) = q_{(xz)'}\).
	
	Consider the case where \(|x| < k\). Then \((xz)' = xz\). By the induction hypothesis, \(w = x\), which implies that \(wz = xz\). This means that \(P_{q_{(xz)'}(wz)}\) is true.

	Then, consider the case where \(|x| = k\). By the induction hypothesis, the last \(k\) characters of \(w\) are \(x\), but by concatenating \(z\), the last \(k+1\) characters of \(wz\) become \(xz\). It follows that the last \(k\) characters of \(wz\) are \((xz)'\), which means that \(P_{q_{(xz)'}}(w)\) is true.

	Thus all state invariants are correct.

	Now, for \(w \in \Sigma ^*\), define \(w_k\) to be the string made of the last \(k\) characters of \(w\). It will be shown that \(\mathcal{D}\) accepts \(L\).

	Let \(w \in L\). Then the \(k\)th last character is a 1. By the induction hypothesis, \(\delta (q_{\varepsilon}, w) = q_{w_k}\). The first character of \(w_k\) is a 1, so \(w_k \in F\) and \(q_{w_k} \in q_F\), so \(w\) is accepted.

	Conversely, suppose that \(w \notin L\). Then the \(k\)th last character of \(w\) is not a 1. By the induction hypothesis, \(\delta (q_{\varepsilon}, w) = q_{w_k}\), but the first character of \(w_k\) is not 1, so \(w_k \notin F\), which means that \(q_{w_k} \notin q_F\), so \(w\) gets rejected.

	Therefore \(\mathcal{D}\) accepts \(L\). Since \(\mathcal{D}\) has \(2^{k+1}\) states, the smallest DFA that accepts \(L\) must have less than that, which completes the proof.
	\smallbreak
\end{proof}
\end{enumerate}
