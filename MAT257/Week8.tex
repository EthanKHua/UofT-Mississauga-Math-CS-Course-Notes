...
\begin{document}
    ...
    \begin{proof}
        ...
        \]
        Since \(C'\) is compact, by the generalized EVT, \(f\) attains a minumum \(m\). Notice that since norms are positive, \(f(\vec{x}) > 0\), so \(m > 0\). We claim that this \(m\) is the value we are looking for. That is, \(m\|\vec{x}\| _1 \leq \|\vec{x}\|\) is true for all \(\vec{x} \in X\).
