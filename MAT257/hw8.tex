...
\begin{document}
    ...
    \begin{question}{22}
        ...
        \tcblower
        Equip \(\ell ^1\) with the 1-norm and \(c_0\) with the sup-norm. Define the function \(f: \ell ^1 \to c_0^*\) by \(f(\vec{x})(\vec{a}) = \sum_{i=1}^{\infty} x_i \cdot a_i\). We claim that this is a bijective isometry.

        Let \(A = \{|f(x)(a) - f(x)(a)| : a \in c_0, \|a\| _{\infty} \leq 1\}\).

        To show that this function is an isometry, let \(x,y \in \ell ^1\). It will be shown that \(\|x-y\| _1 = \sup A\).
        ...
        \[
            \|x-y\| _1 \text{.} 
        \]
        This means that \(\|x-y\| _1\) is an upper bound for \(A\).
    \end{question}
    \pagebreak
    \begin{question}{23}
