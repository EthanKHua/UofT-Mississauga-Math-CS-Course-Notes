...
\begin{document}
    ...
    First, we will show that there is \(\xi > 0\) so that for any polynomial \(g\) with coefficients \(b_i\) and \(\|a_i - b_i\| < \xi\), \(g\) has no repeated roots.

    Suppose for contradiction that for all \(\xi > 0\), there is a \(g\) such that \(\|a_i - b_i\| < \xi\) and \(g\) has a repeated root.

    Let this repeated root be \(r\). If \(r\) is not a root of \(f\), then
    \end{question}
\end{document}