...
\begin{document}
    ...
    \[
        ||f(x)| - |g(x)|| \leq |f(x) - g(x)| = \left\vert \sum_{i=0}^{n} a_i x^i - \sum_{i=0}^{n} b_i x^i \right\vert \leq \sum_{i=0}^{n} |a_i - b_i| |x_i| < \frac{\inf S}{2m} \cdot m = \frac{\inf S}{2}
    \]
    \[
        \implies |f(x)| - \frac{\inf S}{2} < |g(x)| < |f(x)| + \frac{\inf S}{2}
    \]
    \[
        \implies |g(x)| > \frac{\inf S}{2} > 0
    \]
    which means that \(g\) never attains a root outside of \(V\).
    \end{question}
\end{document}