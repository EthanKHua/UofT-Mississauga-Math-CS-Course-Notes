...
\begin{document}
    ...
    Recall the \(\delta\) we got from part (a). We claim that if we shrink it down to \(\eta = \min \{\frac{\delta}{2}, \xi \}\), the number of roots does not change. Let \(g\) be a polynomial with coefficients \(b_i\) such that \(|a_i - b_i| < m\) 
