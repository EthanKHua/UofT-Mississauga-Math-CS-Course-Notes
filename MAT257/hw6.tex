...
\begin{document}
    ...
    \begin{question}{19}
        ...
        \[
            y_{N_i} = \begin{dcases}
                \varepsilon, &\text{ if } (\vec{y}_i)_{N_1} < \frac{\varepsilon}{2};\\
                0, &\text{ if } (\vec{y}_i)_{N_1} \geq \frac{\varepsilon}{2} ;\\
            \end{dcases}
        \]
        For all other terms in \(y\), make it 0.

        Conversely, suppose that \(x_n \rightarrow 0\).
    \end{question}
    % \question{20}
    % \end{proof}
\end{document}