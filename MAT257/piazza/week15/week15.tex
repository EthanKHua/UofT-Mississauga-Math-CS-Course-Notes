...
\begin{document}
    ...
    \begin{proof}
        Let \(U,V \subseteq \mathbb{R}^n\) be open, and let \(\Phi : U \to V\) be a simple diffeomorphism. Without loss of generality, let the only non-simple component of \(\Phi\) be \(\varphi _n\), and let \(E = \prod_{i=1}^n [a_i, b_i] \subseteq U\) be a closed box. Since \(\Phi\) is a diffeomorphism, this implies that \(\Phi (E)\) is measurable, and as well, because \(\Phi\) is simple,
        \[
        ...
        \]
        \(\Phi\) is a diffeomorphism, so the restriction \(\varphi(x', \cdot)\) is also a diffeomorphism. It follows that the image \(\varphi(x', [a_n, b_n])\) is an interval. Now, we want to apply single-variable change of variables with \(\varphi (x', u) = x_n\). Notice that
        \[
        ...
        \[
            \det J \Phi = \begin{array}{|cccc|}
                ...
                0 & 0 & \cdots & \dfrac{\partial \varphi }{\partial u}(x', u) \\
