...
\begin{document}
    ...
    We claim that it suffices to show that the inversion map for invertible linear maps \(T \mapsto T^{-1}\) is smooth.
    ...
    Let \(rS^3 = \{ (x,y,z,w) \in \mathbb{R}^4 : \sqrt{x^2 + y^2 + z^2 + w^2} = r\}\). Let \(S_w^+ = \{ (x,y,z,w) \in \mathbb{R}^4 : \sqrt{x^2 + y^2 + z^2 + w^2} = r, w > 0\}\) be the positive \(w\)-hemisphere of \(rS^3\). We define \(S_w^-, S_z^+, S_z^-, S_y^+, S_y^-, S_x^+, S_x^-\) similarly. It is not too hard to see that these sets form an open cover of \(rS^3\). We will only analyze the volume of \(S_w^+\), as the volume for all other sets can be computed in the same fashion. Let \(f(x,y,z) = \sqrt{r^2 - x^2 - y^2 - z^2}\). We define the parametrization \(\Phi: B(0, 1) \subseteq \mathbb{R}^3 \to S_w^+\) by
    \[
