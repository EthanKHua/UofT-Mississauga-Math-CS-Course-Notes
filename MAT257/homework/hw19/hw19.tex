...
\begin{document}
    ...
    \begin{question}{44}
        ...
        \textbf{Lemma.} From a manifold \(M\), take two charts \((U, \varphi)\), \((V, \psi)\) that are consistently oriented with each other. Then \((U ,\varphi)\) is positively oriented with \(\mathcal{A}^+\) if and only if \((V, \psi)\) is positively oriented with \(\mathcal{A}\).
        ...
        so
        \[
            J (\psi \circ \gamma^{-1}) = J (\psi \circ \varphi^{-1}) \cdot J (\varphi \circ \gamma^{-1})
        \]
        and
        \[
            \det (\psi \circ \gamma^{-1}) > 0
        \]
        as needed.
